...
import { ISummaryProcedureDataDTO } from '@/types/dto/analytics';
import { DISPLAY_FORMAT, getCurrentDate, itechDateToDayjs } from '@/utils/dateUtils';

export const getSummaryProcedureDoc = ({
  data,
  date,
}: {
  data: ISummaryProcedureDataDTO;
  ...
  return {
    ...
    content: [
      ...
      {
        text: 'THỐNG KÊ DỊCH VỤ THEO DỊCH VỤ CHỤP',
        fontSize: 16,
        alignment: 'center',
        italics: true,
        bold: true,
          ...
          body: [
            ...
            ...data.map((item, index) => {
              return [
                ...
                { text: item.procedure.name, style: 'row' },
