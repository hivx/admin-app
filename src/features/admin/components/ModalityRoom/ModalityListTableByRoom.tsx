...
import { TABLE_MODALITY_BY_ROOM } from '@/stores/table/tableInitialState';
...
type ModalityTableProps = {
  modalityRoomID: BaseEntity['id'];
};

export const ModalityListTableByRoom: FC<ModalityTableProps> = (props) => {
  const { modalityRoomID } = props;
  const translate = useTranslate();
  const { data, isFetching, refetch } = useGetListModalityQuery({
    filter: { roomID: modalityRoomID },
  });
  ...
  return (
    <MyTable
      tableId={TABLE_MODALITY_BY_ROOM}
      data={data?.list}
