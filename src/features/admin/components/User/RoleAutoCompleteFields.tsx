...
type RoleAutoCompleteFieldsFieldProps<T extends FieldValues> = {
  name: Path<T>;
  control: Control<T>;
};

export const RoleAutoCompleteFieldsFields = <T extends FieldValues>(
  props: RoleAutoCompleteFieldsFieldProps<T>,
) => {
  ...
  return roleData ? (
    <MyFormAutoComplete
      name={name}
      control={control}
      placeholder={translate.resources.user.role()}
      enableSelectAll
      MyAutoCompleteProps={{
        size: 'small',
        fullWidth: true,
        options: roleData.list,
